  ...
  stages {
    stage('Checkout code') {
     ...
     stage('Restore Nuget') {
      ...
      stage('Build') {
      steps {
        bat "\"${tool 'C:\Program Files (x86)\MSBuild\14.0\Bin\MSBuild.exe'}\" Mantis Automation.sln /p:Configuration=Release /p:Platform=\"Any CPU\" /p:ProductVersion=1.0.0.${env.BUILD_NUMBER}"
      }
      }
      stage('Tests') {
      steps {   
